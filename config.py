...
class Config(object):
    ...
    # # listen to.
    # # Example of a redirection from https://tClinix/terminal1 to port 8080 :
    # # location /terminal1 {
...
class DebugConfig(Config):
    DEBUG = True
    SECRET_KEY = environ.get('TCLINIX_SECRET_KEY', 'get-a-real-key')


class ProductionConfig(Config):
    DEBUG = False
    # In production, the secret MUST be provided as an environment variable.
    SECRET_KEY = environ.get('TCLINIX_SECRET_KEY')

    # Vault
    # In production, all credentials (hashes, usernames and passwords) are
    # stored in a vault.
    # # There MUST be a Vault configured to use tClinix in production mode.
